...
class TestSPMetadataXSD(unittest.TestCase, common.wrap.TestCaseWrap):
    ...
    def tearDownClass(cls):
        fname = '%s/sp-metadata-xsd-ag.json' % DATA_DIR
    ...
    def test_xsd(self):
        '''Validate the SP metadata against the SAML 2.0 Medadata XSD for SPID Aggregator'''

        cmd = ' '.join(['xmllint',
                        '--noout',
                        '--schema ./xsd/saml-schema-metadata-aggregated-spid.xsd',
                        METADATA])
        is_valid = True
